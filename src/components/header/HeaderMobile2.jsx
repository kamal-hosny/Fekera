            ...
            <>
              {item.name === "Search" ? (
                ...
                onClick={toggleSearch}
                  key={item.id}
                  className="flex flex-col items-center justify-center p-3 gap-1 text-sm"
                >
                  <button className="relative">
                    {item.count && (
                      <span
                        key={`count-${item.id}`}
                        className="absolute top-0 right-0 transform translate-x-1/2 -translate-y-1/2 text-slate-50 bg-red-600 h-4 w-4 rounded-full text-xs"
                      >
                        1
                      </span>
                    )}
